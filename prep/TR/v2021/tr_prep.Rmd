---
title: "Tourism (TR) Goal Data Prep"
output:
  github_document:
    toc: true
    toc_depth: 3
...
```{r tr background, child = bkgd_path, results = "asis", echo = FALSE}
```
...
These are saved to the `layers` folder. Saved to `prep/TR/v2021/intermediate` are intermediate datasets: `DATASETINTEMED1.csv` and `DATASETINTEMED2.csv`. All these are derived from or informed by the raw datasets from RAW DATA SOURCES.
...
```{r tr load raw data, echo = TRUE, message = FALSE, warning = FALSE, results = "hide"}
