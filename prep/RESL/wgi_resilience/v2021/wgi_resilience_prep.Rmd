...
These are saved to the `layers` folder. Saved to `prep/RESL/wgi_resilience/v2021/intermediate` are intermediate datasets: `DATASETINTEMED1.csv` and `DATASETINTEMED2.csv`. All these are derived from or informed by the raw datasets from RAW DATA SOURCES.
...
<!-- dataset save location BHI_share/BHI 3.0/Resilience/ -->
...
dir_rawdata <- file.path(dir_B, "Resilience", "wgi_resilience")
```
